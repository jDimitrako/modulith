...
The template provides a flexible caching infrastructure with the following features:

- Shared `ICacheService` interface in the Shared Kernel
- Redis implementation with configurable options
...
public class GetTodosQueryHandler : IRequestHandler<GetTodosQuery, IEnumerable<TodoItemDto>>
{
    ...
    public GetTodosQueryHandler(ICacheService cacheService, IMediator mediator)
    {
    ...
    {
        var cacheKey = CacheKeyGenerator.GenerateListKey<TodoItem>();
        return await _cacheService.GetOrSetAsync(
            cacheKey,
            async () => await _mediator.Send(new GetTodosQuery(), cancellationToken),
            new CacheOptions
            {
...
The template includes comprehensive rate limiting with multiple policies:

1. Global Rate Limiter (100 requests/minute per user/IP)
2. API Rate Limiter (1000 requests/minute per user/IP)
3. Auth Rate Limiter (5 requests/minute per IP)
4. Burst Rate Limiter (20 requests/10 seconds per user/IP)
5. Concurrent Rate Limiter (10 concurrent requests per user/IP)
6. Chunked Rate Limiter (1000 requests/hour per user/IP)
...
The template uses Hangfire for background job processing with the following features:

- Dedicated `Modulith.Worker` project for Hangfire server
- PostgreSQL storage
...
The Hangfire dashboard is accessible at `http://localhost:8080/hangfire` (username: `admin`, password: `admin`).

Example usage:

```csharp
public class AddTodoCommandHandler : IRequestHandler<AddTodoCommand, TodoItemDto>
{
    ...
    public async Task<TodoItemDto> Handle(AddTodoCommand request, CancellationToken cancellationToken)
    {
        ...
        _jobClient.Enqueue<ITodoNotificationService>(x => 
            x.SendTodoCreatedNotificationAsync(todo.Id, todo.Title));

        // Schedule reminder
        _jobClient.Schedule<ITodoNotificationService>(x => 
            x.SendTodoReminderNotificationAsync(todo.Id, todo.Title),
...
1. **Caching**
   - Use appropriate cache keys with prefixes
...
2. **Rate Limiting**
   ...
   - Use burst limiting for high-traffic endpoints
   - Monitor rate limit hits
   - Provide clear error messages
   - Consider user roles in rate limiting
...
3. **Background Jobs**
   ...
   - Use continuations for complex workflows
...
- **Multiple Version Support**: APIs can be versioned using URL segments (e.g., `/api/v1/todos`), headers,